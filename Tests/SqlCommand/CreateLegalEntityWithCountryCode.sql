IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'CreateLegalEntityWithCountryCode')
BEGIN
    EXEC('CREATE PROCEDURE CreateLegalEntityWithCountryCode
    -- required paramters
...
BEGIN
	...
	INSERT INTO PRBankAccount (ShortName,LongName,RoutingNumber,AccountNumber,FractionalNumber,CheckAuthorizingMessage1, CheckSigning,XRefCode,ClientId,LastModifiedUserId,LastModifiedTimestamp,UseForPrintingCheck,MICRIsBold)
	VALUES(''AssociatedBank'', null ,''075900575'',123456789,''123123'',''Payroll Check'',0, null ,@clientId,@cadminId,CURRENT_TIMESTAMP,0,1)
